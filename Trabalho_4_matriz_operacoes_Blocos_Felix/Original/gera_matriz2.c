...
int main(int argc, char **argv) {
  ...
  int **mat1 = NULL;
	...
	if (argc != 4){
		printf ("ERRO: Numero de parametros %s <nr_linhas> <nr_colunas> <matriz_a ou b>", argv[0]);
		exit (1);
	}
	...
	coluna = atoi(argv[2]);
	//matriz = atoi(argv[3]);
	sprintf (filename, "%dx%d-mat%s.map", linha, coluna, argv[3]);
  ...
  if (fmat0 == NULL) {
	...
	//printf("#####\n Arquivo com a matriz gerada (%dx%d-mat%s.map).\n#####\n", linha, coluna, argv[3]);
  ...
  fclose(fmat1);
	...
	printf("##### Arquivo %dx%d-mat%s.map: VERIFICADO! #####\n", linha, coluna, argv[3]);
  return 0;
}
